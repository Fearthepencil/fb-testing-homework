def test_destination_page_elements(destination_page):
    ...
    assert (
        destination_page.expected_tooltip_text in tooltip_text
    ), "Wishlist tooltip is not valid"

    see_availability_button = destination_page.get_see_availability_button(
        first_charter_card
    )
    assert (
        see_availability_button.is_visible()
    ), "See availability button is not visible"
    assert (
        see_availability_button.is_enabled()
    ), "See availability button is not clickable"
