...
class DestinationPage(BasePage):
    ...
    BOAT_LENGTH_SELECTOR = "[data-testid='charter-card-boat-silhouette'] p:has-text('ft')"
    MAX_PEOPLE_SELECTOR = "[data-testid='charter-card-boat-silhouette'] + p:has-text('people')"
    ...
    def __init__(self, page, test_config: dict):
        ...
        self.expected_tooltip_text = test_config["test_requirements"]["expected_tooltip_text"]
    
    def wait_for_charter_cards(self, timeout: int = 10000):
        # Wait for at least minimum number of charter cards
        self.page.wait_for_selector(
            self.CHARTER_CARD_SELECTOR,
            timeout=timeout,
            state="visible"
        )
    ...
    def get_boat_length(self, card_locator):
        ...
        boat_container = card_locator.locator("[data-testid='charter-card-boat-silhouette']").first
