...
class SitemapPage(BasePage): 
    ...
    TOP_DESTINATIONS_SECTION_SELECTOR = "h1[class*='list-title']:has-text('Top Fishing Destinations')"
    ...
    def wait_for_top_destinations_section(self, timeout: int = 5000) :
        self.page.wait_for_selector(
            self.TOP_DESTINATIONS_SECTION_SELECTOR,
            timeout=timeout,
            state="visible"
        )
        self.page.wait_for_selector(
            self.FIRST_DESTINATION_LINK_SELECTOR,
            timeout=timeout,
            state="visible"
        )
